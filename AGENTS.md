...
-
- If `state.json` or `00_System_Rules.json` is missing in `Campaign/`, create them from templates:
...
- If `Campaign/state.json` sets **Assistant Preferences → Preferred Language**, generate all user-facing chat in that language (e.g., Ukrainian).
