...
1.  **Review the Target (SessionPlanner Mode):**
    Read `state.json` to determine the *Current Episode* and *Current Chapter*. Read the corresponding `03_Story/.../Encounter.json` files or previous session logs to understand the starting point. Identify where the PCs currently are and what unresolved plot threads exist.
...
5.  **Mechanical Setup (Marauder Mode):**
    Identify the key NPCs or monsters required for the Encounters outlined in Step 3. Switch to the **Marauder** (RulesLawyer) persona and verify if those NPCs exist in `02_Characters/`. If they do not, ask the GM if they want you to generate the stat blocks now.

6.  **Flavor Generation (KingCrab Mode):**
    Switch to the **KingCrab** (Narrator) persona and generate 1-2 paragraphs of read-aloud boxed text for the opening scene of the session to give the GM a strong starting point.
