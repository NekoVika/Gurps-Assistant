...
5.  **Update Campaign Overview:**
    Modify the `03_Story/Campaign_Overview.json` file to add a link to the new Episode in the Episode Index.

6.  **Update Global State (Conditional):**
    Ask the GM if this is the new active Episode they are currently playing, or if they are prepping ahead. If active, modify `state.json` to update the "Current Episode" to the new episode and set "Current Chapter" to "None (Starting Episode)". If prepping ahead, leave `state.json` alone.
