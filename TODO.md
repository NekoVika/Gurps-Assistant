...
- [x] Differentiate existing vs new entities: The model currently invents new relations, child encounters (e.g., "Drone Defense"), and factions perfectly, but this confuses the GM because no such files exist yet. *(Shared name-resolution rule — `src/gurpsai/app/services/link_resolver.py` mirrored by `web/src/lib/entityResolution.ts` — normalizes case/underscores/ordinals and powers `/campaign/validate`'s structured `dangling` report with suggested types.)*
- [x] UI visual cues for "Draft/Unlinked" entities: If the AI generates an array of `childLinks` or `characters` that do not map to existing files, the UI should clearly highlight them as "Proposed/Missing" so the GM knows they need to be created. *(Amber "(proposed)" badges on InternalLink in all passports, ghost rows in the registry tree, a MissingEntitiesBanner on story passports, and a dangling-references group in the validation panel.)*
- [x] Batch creation workflows: Provide the GM with a 1-click way to generate stubs for any new entities the AI just hallucinated into the campaign. *(One-click stub creation from ghost rows, proposed links, the story-passport banner, and "Create all missing stubs" in the validation panel; `/campaign/stubs/batch` now lands stubs in registry-visible canonical folders and supports `parent_path` for nested story elements.)*
